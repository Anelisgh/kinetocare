...
public class KeycloakProxyController {
    ...
    public KeycloakProxyController(WebClient.Builder webClientBuilder) {
    ...
    public Mono<ResponseEntity<Object>> handleTokenRequest(ServerWebExchange exchange) {
        return exchange.getFormData() // extrage campurile trimise in body
                .flatMap(formData -> { // flatMap -> lanseaza o actiune dupa ce datele sunt gata
                    ...
                    // POST request reactiv catre keycloak
                    return webClient.post()
                            .uri(getTokenEndpoint())
                            .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                            .body(BodyInserters.fromFormData(keycloakParams))
                            .exchangeToMono(response -> {
                                ...
                                } else if (response.statusCode().equals(HttpStatus.BAD_REQUEST) || response.statusCode().equals(HttpStatus.UNAUTHORIZED)) {
                                    return buildError((HttpStatus) response.statusCode(), "Email sau parolă incorectă");
                                } else {
                                    return buildError((HttpStatus) response.statusCode(), "Eroare de la Keycloak");
                                }
                            })
                            .onErrorResume(e -> {
                                if (e instanceof WebClientResponseException wce) {
                                    if (wce.getStatusCode() == HttpStatus.UNAUTHORIZED) {
    ...
    private Mono<ResponseEntity<Object>> buildError(HttpStatus status, String message) {
        Map<String, Object> body = Map.of(
                "error", status.getReasonPhrase(),
                "message", message
        );
        ResponseEntity<Object> entity = ResponseEntity.status(status).contentType(MediaType.APPLICATION_JSON).body(body);
