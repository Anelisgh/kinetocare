...
Această secțiune prezintă algoritmul de tip *Greedy* utilizat pentru determinarea și generarea sloturilor orare disponibile în agenda terapeuților. Sunt analizate fazele premergătoare de colectare a constrângerilor, implementarea logicii bazate pe ferestre glisante cronologice și evaluarea complexității asimptotice.
...
După ce toate datele de intrare și constrângerile au fost colectate, algoritmul *Greedy* inițializează un cursor temporal setat la ora de început a programului terapeutului (`orar.oraInceput()`). Logica de funcționare a ferestrei glisante (*sliding window cursor*) este descrisă în algoritmul conceptual următor:
